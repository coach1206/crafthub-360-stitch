...
// Routes 1-21 need journey state seeded
...
;(async () => {
  ...
  for (const vp of VIEWPORTS) {
    ...
    // wrapper-strength redirect check
    await page.goto(`${BASE}/smokecraft`, { waitUntil: 'domcontentloaded' })
    await page.evaluate(() => sessionStorage.setItem('novee_demo_mode', '1'))
    ...
    const redirectOk = finalUrl.includes('/smokecraft/seed-soil')
    console.log(`  ${redirectOk ? '✓' : '✗'} wrapper-strength → ${finalUrl}`)
    await page.screenshot({ path: `${DIR}/wrapper-strength-redirect--${vp.name}.png`, fullPage: false })

    await ctx.close()
  }

  await browser.close()
  console.log('\n✅ All 28-route screenshots captured at 4 viewports.')
})()
