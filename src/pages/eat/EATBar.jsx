...
export default function EATBar() {
  const tickets = getTickets().filter((t) => t.status !== 'paid')
  const barItems = tickets.flatMap((t) => t.items.filter((i) => i.station === 'bar').map((i) => ({ ...i, ticketId: t.id, tableId: t.tableId })))
  const stock = EAT_INVENTORY.filter((i) => i.station === 'bar')

  return (
    <ManagementLayout title="Bar" subtitle="Active bar tickets and spirits stock">
      <div style={{ marginBottom: 18, fontWeight: 700 }}>Active Bar Orders</div>
      <Table
        columns={[{ key: 'name', label: 'Item' }, { key: 'qty', label: 'Qty' }, { key: 'ticketId', label: 'Ticket' }, { key: 'tableId', label: 'Table' }]}
        rows={barItems}
        renderCell={(r, k) => r[k]}
      />
      <div style={{ margin: '20px 0 10px', fontWeight: 700 }}>Bar Stock</div>
      <Table
