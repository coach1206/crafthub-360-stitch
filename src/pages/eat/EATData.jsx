...
import { ManagementLayout, Card, Table, Pill, Btn, GOLD } from '../../components/eat/ui.jsx'
...
function EmptyDataState({ events }) {
  ...
  return (
    <Card style={{ marginBottom: 16 }}>
      <div style={{ fontWeight: 700, marginBottom: 6, color: '#e8eef5' }}>No live data yet</div>
      <div style={{ fontSize: 13, color: '#8b95a3', marginBottom: 14 }}>
      ...
      <div style={{ fontSize: 11, fontWeight: 800, color: GOLD, letterSpacing: '0.08em', marginBottom: 8 }}>DATA SOURCES</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
        {DATA_SOURCES.map((s) => (
          <div key={s.key} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#cdd5df' }}>
            <span>{s.label}</span>
            <Pill label={seenSystems.has(s.key) ? 'reporting' : 'no events yet'} tone={seenSystems.has(s.key) ? 'open' : 'dirty'} />
          </div>
        ))}
      </div>
      <Btn tone="purple" onClick={() => navigate('/eat/operations')} style={{ padding: '7px 12px', fontSize: 12 }}>Open Operations View</Btn>
    </Card>
  )
}
