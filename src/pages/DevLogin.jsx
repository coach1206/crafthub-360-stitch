/**
 * Developer Login — T014
 * Email + PIN authentication for NOVEE OS Developer role.
 * Requires an active developer access grant issued by Founder L0.
 * Sidecar role — permission-gated access to diagnostics only.
 */
...
export default function DevLogin() {
  ...
  const submit = async () => {
    ...
    setLoading(true)
    setError('')
    const result = await loginDev(email.trim().toLowerCase(), pin)
    if (result.success) {
      window.location.href = '/dev-diagnostics'
    } else {
      setError(result.error || 'Invalid credentials or no active developer grant.')
      setPin('')
    }
    setLoading(false)
  }
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <div style={{
          ...
          <span className="material-symbols-outlined" style={{ fontSize: '24px', color: GOLD }}>terminal</span>
        </div>
        <div style={{ color: DIM, fontSize: '10px', letterSpacing: '0.3em', marginBottom: '0.4rem' }}>
          NOVEE OS — SYSTEM ACCESS
        </div>
        <h1 style={{
        ...
        <div style={{ color: 'rgba(201,168,76,0.2)', fontSize: '11px', marginTop: '0.4rem', letterSpacing: '0.1em' }}>
          Requires active developer access grant
        </div>
      </div>
      ...
      <div style={{
        ...
        <span className="material-symbols-outlined" style={{ fontSize: '14px', color: 'rgba(201,168,76,0.35)', marginTop: '1px', flexShrink: 0 }}>
          info
        </span>
        <span style={{ color: 'rgba(201,168,76,0.3)', fontSize: '10px', letterSpacing: '0.08em', lineHeight: 1.6 }}>
          Access is logged. Read-only diagnostics only — no guest data, no credentials, no payments.
        </span>
      </div>
      ...
      <div style={{
        ...
        {error && (
          <div style={{
            ...
            fontSize: '12px', padding: '8px 12px', textAlign: 'center', marginBottom: '1rem',
          }}>
            {error}
          </div>
        )}
      ...
      <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
        <span style={{ color: 'rgba(201,168,76,0.15)', fontSize: '10px', letterSpacing: '0.1em' }}>
          Access granted by Founder Level 0 only
        </span>
      </div>
    </div>
  )
