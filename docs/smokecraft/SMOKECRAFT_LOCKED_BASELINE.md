...
Three brand-new staff screens (`VenueHumidorAdminDashboard.jsx`,
`VenueHumidorAdminProductForm.jsx`, `VenueHumidorAdminInventoryEvents.jsx`)
...
restyled. Live route count is now 117 (`SMOKECRAFT_GAME_MANIFEST.json`
...
Two brand-new screens (`VenueHumidorHandoff.jsx`,
`VenueHumidorPickup.jsx`) were added in the same locked navy/
champagne-gold `SmokeCraftScreenShell` system — no existing approved
SmokeCraft screen was touched or restyled. `VenueHumidorOrderDetail.jsx`
gained new controls (Block/Unblock/No-Show/Expire/Verify & Handoff)
within its existing layout, not a redesign. Live route count is now
122 (`SMOKECRAFT_GAME_MANIFEST.json` regenerated from a fresh
...
covering all 122 routes, and `validateSmokecraftResponsive.mjs` passes
0 failed. Payment integration was not built, per mandate.

## Venue Humidor 1B-2B-4 update

Five brand-new screens (`VenueHumidorMyOrders.jsx`,
`VenueHumidorMyOrderDetail.jsx`, `VenueHumidorReceipt.jsx`,
`VenueHumidorMyAcquisitions.jsx`, `VenueHumidorAcquisitionDetail.jsx`)
were added in the same locked navy/champagne-gold `SmokeCraftScreenShell`
system — no existing approved SmokeCraft screen was touched or
restyled. Live route count is now 127
