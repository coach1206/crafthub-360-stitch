...
## SC-D014 — Holistic Fix 2E-5 finding: mid-spine screens with backend-dependent Continue actions cannot be verified by a localStorage-only test harness

While building `verify-smokecraft-hf2e5-curriculum-forward-backward.mjs`, a real click on Flavor Memory's (Session 10) "Continue" control did not advance the session. Root-caused via source read: `FlavorMemory.jsx`'s `handleContinue()` calls `saveToBackend()`/`saveToPassport()` (real `fetch()` calls to `/api/modules/smokecraft/pairing/flavor-memory` and `/api/passport-360/sync/flavor-memory`) and only proceeds to `awardSessionRewards`/navigate if both resolve `ok`. Manually confirmed both endpoints return non-2xx (400/401) when called without the cookie/session state a real signed-up guest would hold — the dedicated test's localStorage-only seeding (matching the pattern used throughout this operation for lightweight state seeding) does not establish that backend session, so the click legitimately hits a real save-failure path rather than a UI bug. **Severity: unknown/unconfirmed** — this could mean (a) the test harness needs a real signup flow before testing backend-dependent screens (most likely, given `full-journey-sequence-and-assets.mjs`'s proven Welcome→Session 2 click already works without this problem because that transition has no backend dependency), or (b) a real guest player's session could hit the same save failure under some condition not yet identified. Not fixed this pass — flagged for the next test-harness or backend-session investigation. The dedicated forward/backward test was adjusted to use the already-proven Welcome→Session 2 transition instead, to avoid this specific screen while the root cause of (a) vs (b) is unresolved.
