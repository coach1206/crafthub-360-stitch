...
- **Welcome — FIXED (final closeout pass).** Learner/venue/status, Cigar Preview, Mentor Preview, plus two new cards (Session 1 Preview, Golden Box Objective Preview) are now real expandable controls (`SmokeCraftTactileCard`-style expand/collapse pattern via a shared `SummaryCard`), each opening a real detail explanation (what/why/journey-impact/Golden Box relevance/next-step). No panel open by default. Cigar/Mentor cards are `disabled` (not clickable) when no real selection exists, never showing fake data. Live-verified: clicking the Cigar Preview sets `aria-expanded="true"`.
- **Lighting Tutorial — FIXED (final closeout pass).** Already had a real 8-step interactive sequence with mentor/knowledge tips and a Continue-gated-on-all-viewed rule (this was mis-flagged as "0 selectable-controls" by the prior grep-based heuristic, which didn't recognize `aria-current="step"` progress-dot buttons as selectable). The one genuine gap — step progress was component-local `useState` only, lost on refresh — is now fixed: `stepIndex`/`viewedSteps` persist to `journey.cutToastLight.lightingTutorialProgress`. Live-verified: progress survives a real page reload.
- **Mentor Commentary — FIXED (final closeout pass).** The three advice sections (Construction/Flavor/Suggested Action) are now individually expandable and support a real "Apply to my journey" action gated behind an explicit Confirm/Cancel step (never auto-applies) plus a Dismiss/Restore toggle. Journey-persisted per mentor id so a different mentor on a later journey never inherits a prior mentor's applied advice. Live-verified: clicking Apply shows a real confirmation prompt before anything persists.
