...
- `src/constants/smokecraftAssets.js` — one new key, `recommendedNextJourney`, registering the approved visual shell (`Recommend next journey.png`, uploaded upstream mid-package and merged in — see Known Limitations).
- `verify-all-smokecraft-assets.mjs` — updated the pre-existing Session Complete asset-check entry's expected image path from the prior `SESSION COMPLETE.png` to the new `Recommend next journey.png`, since this package legitimately changed which approved asset that route requests.
...
**Approved image connected:** the literal new asset "Recommend next journey.png" named in this package's brief was not present in the repository when this package began (confirmed via full-filesystem search, same situation as Packages P and R) and was initially wired to fall back to the existing approved Session Complete shell. Partway through this package, the user uploaded it directly to the remote branch (along with several other packages' previously-missing approved images, in two upstream commits: "SmokeCraft production images - Batch 1/2"). Those two commits were fetched and merged into this branch with no conflicts, and `SC_ASSETS.recommendedNextJourney` was then updated to point at the real `Recommend%20next%20journey.png` asset, reused as a decorative header banner; every changing value (identity, XP, progress, recommendation titles, challenge counts, rewards, selected state, buttons, completion state) remains live React content, never baked into the image.
