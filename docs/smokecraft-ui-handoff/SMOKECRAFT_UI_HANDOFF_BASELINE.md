...
**Baseline commit**: the commit titled "SmokeCraft UI handoff closure: fix Scorecard interaction and certify Golden Box gating" on branch `recovery/smokecraft-codex-final` (the final pushed commit of this pass — see the session's final report for the exact SHA, since a commit cannot embed its own hash).
