...
const pages_to_check = [
  ...
  { url: '/smokecraft/management-sync', id: 'management-sync' },
]
  ...
  const info = await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button, a[href]'))
      ...
      .map(el => {
        ...
        return { text: el.textContent?.trim().slice(0, 25), x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: r.height }
      })
