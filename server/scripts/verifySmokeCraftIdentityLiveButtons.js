...
console.log('\nGate 5 — Non-blocking: awardSessionRewards in try/catch, never awaited')
if (identity) {
  check('awardSessionRewards called (session enrollment)',
    identity.includes("awardSessionRewards('enroll')") || identity.includes('awardSessionRewards'))
  check('awardSessionRewards NOT awaited (non-blocking)',
    !identity.match(/await\s+awardSessionRewards/))
  check('awardSessionRewards wrapped in try/catch',
    identity.includes('try') && identity.includes('awardSessionRewards'))
  check('navigate() called after (not blocked by) awardSessionRewards',
