...
if (identity) {
  ...
  check('Identity awards enroll session on click',
    identity.includes("awardSessionRewards('enroll')"))
  check('Identity uses triggerHaptic for tactile feedback',
    identity.includes('triggerHaptic'))
  check('Identity navigation is non-blocking (not await on awardSessionRewards)',
    !identity.match(/await\s+awardSessionRewards/))
  ...
  check('Identity awardSessionRewards wrapped in try/catch (non-blocking)',
    identity.includes('try') && identity.includes('awardSessionRewards'))
...
// ── Gate 10: Ticket Tapper animated ──────────────────────────────────────────
console.log('\nGate 10 — Ticket Tapper: marquee animation + placeholder')
const ticker = read('src/components/smokecraft/TicketTapperSpecialsStrip.jsx')
check('TicketTapperSpecialsStrip.jsx exists', ticker !== null)
if (ticker) {
  check('Marquee animation (sc-ticker-scroll) defined', ticker.includes('sc-ticker-scroll'))
  check('Empty state placeholder present', ticker.includes('coming soon') || ticker.includes('Specials coming'))
  check('Hover pauses ticker animation', ticker.includes('animation-play-state: paused'))
  check('Reduced motion handled', ticker.includes('prefers-reduced-motion') || ticker.includes('sc-ticker'))
}
