...
check('PairingLab has named hotspot or CTA button', pairingLab
  ? (pairingLab.includes('label:') || pairingLab.includes('navigate(') || pairingLab.includes('onClick'))
  : false)
const finalReview = read('src/pages/smokecraft/FinalReview.jsx')
check('FinalReview has named hotspot or CTA button', finalReview
  ? (finalReview.includes('label:') || finalReview.includes('navigate(') || finalReview.includes('onClick'))
  : false)
const requestPurchase = read('src/pages/smokecraft/RequestPurchase.jsx')
check('RequestPurchase has at least one hotspot or CTA',
  requestPurchase
    ? ((requestPurchase.match(/label:/g) || []).length >= 1 ||
       requestPurchase.includes('navigate(') || requestPurchase.includes('onClick'))
    : false)
