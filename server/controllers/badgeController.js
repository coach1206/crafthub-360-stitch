/**
 * Badge Controller
 * Badge catalog and per-user unlock state (in-memory).
 */
...
export function getCatalog(_req, res) {
  success(res, { badges: BADGE_CATALOG, total: BADGE_CATALOG.length })
}

export function getUserBadges(req, res) {
  const { userId } = req.params
  const earned = getOrCreateSet(userId)
  const badges  = BADGE_CATALOG.map(b => ({ ...b, earned: earned.has(b.id) }))
...
export function unlockBadge(req, res) {
  ...
  const earned = getOrCreateSet(userId)
  if (earned.has(badgeId)) return success(res, { badge, alreadyEarned: true }, 'Badge already earned')
  earned.add(badgeId)
  unlockLog.push({ id: uuidv4(), userId, badgeId, ts: new Date().toISOString() })
  success(res, { badge, alreadyEarned: false, xpAwarded: badge.xpReward }, 'Badge unlocked')
}
