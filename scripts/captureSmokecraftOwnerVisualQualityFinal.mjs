...
const OUT = 'public/proof/smokecraft-owner-visual-quality-final'
...
async function main() {
  ...
  await page.waitForSelector('text=Alpha Lounge (Seed)', { timeout: 20000 }).catch(() => {})
  ...
  if (await alphaLounge.count().catch(() => 0)) {
    await alphaLounge.click()
    await page.click('text=Continue to Welcome')
  } else {
    await page.click('text=Continue without venue')
  }
  ...
  if (!reachedMentorCommentary) {
    await page.goto(`${BASE}/smokecraft/mentor-commentary`, { waitUntil: 'load', timeout: 40000 }).catch(() => {})
  }
