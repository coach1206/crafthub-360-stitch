...
const DIR = 'public/proof/smokecraft-true-final-visual-audit'
...
async function buildSheet(entries, sheetIndex, totalSheets) {
  ...
  const titleSvg = `<svg width="${width}" height="60">
    ...
    <text x="20" y="38" font-family="monospace" font-size="24" fill="#E9C176">SMOKECRAFT TRUE FINAL VISUAL AUDIT — Sheet ${sheetIndex} of ${totalSheets}</text>
  </svg>`

  const base = sharp({ create: { width, height, channels: 3, background: '#060810' } })
  const out = `${DIR}/SMOKECRAFT_TRUE_FINAL_VISUAL_AUDIT_${String(sheetIndex).padStart(2, '0')}.png`
...
async function buildIndex(entries) {
  ...
  const titleSvg = `<svg width="${width}" height="50"><rect width="100%" height="100%" fill="#060810"/><text x="16" y="32" font-family="monospace" font-size="20" fill="#E9C176">SMOKECRAFT TRUE FINAL VISUAL AUDIT INDEX — 001 -&gt; ${String(entries.length).padStart(3,'0')}</text></svg>`
  const out = `${DIR}/SMOKECRAFT_TRUE_FINAL_VISUAL_INDEX.png`
