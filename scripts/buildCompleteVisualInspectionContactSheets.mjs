#!/usr/bin/env node
// Builds the master contact sheets + a small bird's-eye index for the
// SmokeCraft complete owner visual inspection. Pure image composition
// over already-captured real screenshots — no new content.
import sharp from 'sharp'
import { readFileSync, readdirSync, writeFileSync } from 'fs'

const DIR = 'public/proof/smokecraft-owner-complete-visual-inspection'
...
async function buildSheet(entries, sheetIndex, totalSheets) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const line1 = `#${String(entry.n).padStart(3, '0')}  S:${entry.session ?? '-'} P:${entry.phase ?? '-'}  ${f.liveClass || '?'}`
    const line2 = `${entry.name}${entry.state ? ' (' + entry.state + ')' : ''}`.slice(0, 46)
    const line3 = (f.flags || []).join(', ').slice(0, 46)
    ...
    const labelSvg = `<svg width="${THUMB_W}" height="${LABEL_H}">
      ...
      <text x="4" y="42" font-family="monospace" font-size="10" fill="#ff9d6c">${esc(line3)}</text>
    </svg>`
  ...
  const titleSvg = `<svg width="${width}" height="60">
    ...
    <text x="20" y="38" font-family="monospace" font-size="24" fill="#E9C176">SMOKECRAFT COMPLETE VISUAL AUDIT — Sheet ${sheetIndex} of ${totalSheets}</text>
  </svg>`

  const base = sharp({ create: { width, height, channels: 3, background: '#060810' } })
  const out = `${DIR}/SMOKECRAFT_COMPLETE_VISUAL_AUDIT_${String(sheetIndex).padStart(2, '0')}.png`
...
async function buildIndex(entries) {
  ...
  const titleSvg = `<svg width="${width}" height="50"><rect width="100%" height="100%" fill="#060810"/><text x="16" y="32" font-family="monospace" font-size="20" fill="#E9C176">SMOKECRAFT COMPLETE VISUAL INDEX — 001 -&gt; ${String(entries.length).padStart(3,'0')}</text></svg>`
  const out = `${DIR}/SMOKECRAFT_COMPLETE_VISUAL_INDEX.png`
